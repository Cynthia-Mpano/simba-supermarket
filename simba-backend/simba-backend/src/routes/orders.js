...
router.post('/', validateOrder, async (req, res, next) => {
  ...
  try {
    ...
    // 9. Trigger mobile money payment if selected
    let momoReference = null;
    if ((paymentMethod === 'momo' || paymentMethod === 'airtel') && config.MTN_MOMO_PRIMARY_KEY) {
      try {
        momoReference = await requestPayment({
          ...
          provider: paymentMethod, // 'momo' | 'airtel'
        });
        await query(
        ...
        console.error(`${paymentMethod} payment request failed (non-blocking):`, momoErr.message);
      }
    }
