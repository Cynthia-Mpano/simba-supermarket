...
import { ShoppingCart, Search, Menu, X, Sun, Moon, Globe, ChevronDown, User, LogOut, ShoppingBag } from 'lucide-react';
...
export function Header({ onSearch, searchQuery: propSearchQuery }: HeaderProps) {
  const { cartCount, locale, setLocale, isDark, toggleTheme, user, signOut, searchQuery: ctxSearchQuery, setSearchQuery } = useStore();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const router = useRouter();
  const t = (key: Parameters<typeof getTranslation>[1]) => getTranslation(locale, key);

  // Use prop-based search if provided (legacy), otherwise use context
  const currentQuery = propSearchQuery !== undefined ? propSearchQuery : ctxSearchQuery;
                ...
                <DropdownMenuContent align="end" className="w-48">
                  ...
                  <DropdownMenuItem asChild>
                    <Link href="/dashboard" className="flex items-center">
                      <ShoppingBag className="h-4 w-4 mr-2" /> Dashboard
                    </Link>
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
