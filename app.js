...
function navigate(page) {
  ...
  if (page === 'home') {
    ...
    // representative gate authentication check
    if (!state.user || state.user.role !== 'representative') {
      showToast('Unauthorized access. Please login as representative.', 'error');
      navigate('home');
      return;
    }
...
// Render dynamic category filter selector chips
function renderCategoryChips() {
  ...
  categories.forEach(cat => {
    ...
    // translate All chip
    if (cat === 'All') {
      chip.textContent = state.language === 'en' ? 'All Categories' : state.language === 'rw' ? 'Ibyiciro Byose' : 'Toutes les catégories';
    } else {
...
function updateAuthNavigation() {
  if (state.user) {
    ...
    // Toggle navigation visibility of Dashboard for representatives
    if (state.user.role === 'representative') {
      el.dashboardNavBtn.style.display = 'flex';
    } else {
      el.dashboardNavBtn.style.display = 'none';
    }
  } else {
    el.accountNavBtn.innerHTML = `
...
// Product Details Modal Overlay loader
function openProductModal(productId) {
  ...
  body.innerHTML = `
    <div class="modal-body-layout">
      <div style="width: 100%; height: 260px; border-radius: var(--radius-md); overflow: hidden; background-color: var(--bg-main);">
        <img src="${product.image}" alt="${product.name}" style="width: 100%; height: 100%; object-fit: cover;">
      </div>
      <div style="display: flex; flex-direction: column; justify-content: space-between;">
        <div>
          <span style="font-size: 11px; font-weight: 700; color: var(--primary-color); text-transform: uppercase; letter-spacing: 0.5px;">${product.category}</span>
          <h3 style="font-size: 18px; font-weight: 800; color: var(--text-dark); margin-top: 4px; line-height: 1.3;">${product.name}</h3>
          
          <div style="font-size: 20px; font-weight: 800; color: var(--text-dark); margin-top: 10px;">
            ${formatNumber(product.price)} RWF <span style="font-size: 13px; font-weight: 500; color: var(--text-medium);">/ ${product.unit}</span>
          </div>
          
          <div style="margin-top: 14px;">
            <h5 style="font-size: 11px; font-weight: 700; color: var(--text-medium); margin-bottom: 6px;">Available at Simba Branches:</h5>
            <div style="display: flex; flex-wrap: wrap; gap: 4px; max-height: 80px; overflow-y:auto; padding-right:4px;">
              ${product.branches.map(b => `<span style="font-size: 10px; font-weight: 600; padding: 2px 8px; background-color: var(--primary-light); color: var(--primary-color); border-radius: 10px;">${b}</span>`).join('')}
            </div>
          </div>
        </div>
        ...
        <div style="margin-top: 24px; display: flex; gap: 12px; align-items: center;">
          ${product.inStock 
            ? `<button class="btn btn-primary" style="flex: 1; padding: 10px 16px; font-size:13px;" onclick="addToCartById(${product.id}); closeProductModal();">
                 Add to Cart ${inCartQty > 0 ? ` (${inCartQty})` : ''}
               </button>`
            : `<button class="btn" style="flex: 1; background-color: var(--text-light); color: white; cursor: not-allowed; padding: 10px 16px; font-size:13px;" disabled>Sold Out</button>`
          }
        </div>
      </div>
    </div>
